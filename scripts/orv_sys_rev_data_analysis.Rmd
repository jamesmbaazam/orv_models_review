...
```{r}
author_location_count_by_disease_wide <- compact_data_no_fmd %>%
...
#  filter(author_in_country_studied != 'not_applicable') %>% 
  ...
  count(author_in_country_studied) %>% 
  ...
  mutate(yes = ifelse(is.na(yes), 0, yes),
         ...
         total = yes + no + not_applicable
         ) %>% 
  arrange(desc(total))


knitr::kable(author_location_count_by_disease_wide)
```


```{r}
author_location_count_by_disease_long <- author_location_count_by_disease_wide %>% 
...
plot(author_location_count_by_disease_plot)
```
...
disease_studied_by_collab_type <- compact_data_no_fmd %>% 
  ...
  mutate(purely_academic = ifelse(is.na(purely_academic), 0, purely_academic),
         mixed = ifelse(is.na(mixed), 0, mixed),
         total = purely_academic + mixed
         ) %>% 
  arrange(desc(total))
...
disease_studied_by_outbreak_type <- compact_data_no_fmd %>% 
  ...
  mutate(hypothetical_outbreak = ifelse(is.na(hypothetical_outbreak), 0, hypothetical_outbreak),
         real_outbreak = ifelse(is.na(real_outbreak), 0, real_outbreak), 
         total = hypothetical_outbreak + real_outbreak) %>% 
  arrange(desc(total))
...
disease_studied_by_objective_type <- compact_data_no_fmd %>% 
  ...
  mutate(future = ifelse(is.na(future), 0, future),
         past = ifelse(is.na(past), 0, past), 
         both = ifelse(is.na(both), 0, both), 
         total = future + past + both
         ) %>% 
  arrange(desc(total))
