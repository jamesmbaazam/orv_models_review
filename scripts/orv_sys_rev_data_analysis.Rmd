...
collabs_per_year <- compact_data_with_citation_keys_no_fmd %>%
    filter(collab_type == 'mixed') |> 
    count(year_aggreg, collab_type, name = 'total_publications') |> 
    mutate(contribution = total_publications/sum(total_publications), 
           contribution_cumulative = cumsum(contribution), 
           past_half = ifelse(contribution_cumulative <= 0.5, 'no', 'yes')
           ) 
