...
```{r echo=FALSE, fig.width=8, fig.width=8}
#' Count the number of each author affiliation type per year
collabs_per_year <- compact_data_no_fmd %>% 
  count(year_aggreg, 
        collab_type, 
        name = 'total_publications'
        ) 
...
```{r echo=FALSE}
vax_impact_conclusions <- compact_data_no_fmd %>% 
...
model_structure_and_calibration <- compact_data_no_fmd %>% 
  group_by(collab_type) %>% 
  count(parametrization)

knitr::kable(model_structure_and_calibration)

ggplot(data = model_structure_and_calibration) +
  geom_bar(aes(x = parametrization, 
               y = n, 
               fill = collab_type
               ), 
           stat = 'identity',
...
code_availability <- compact_data_no_fmd %>% 
  group_by(collab_type) %>% 
