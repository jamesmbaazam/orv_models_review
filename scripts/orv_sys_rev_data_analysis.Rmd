...
#Save the files 
ggsave(prisma_chart,
       filename = 'outbreak_size_isocline.tiff',
       path = save_fig_here,
       width = 23.76,
       height = 17.86,
       units = 'cm')
...
ggsave(filename = 'publications_per_year.tiff',
...
ggsave(filename = 'collab_proportions.tiff',
       plot = collab_proportions_plot,
       device = 'tiff',
       path = save_fig_here
       )
...
ggsave(filename = 'total_collabs_per_year_plot.tiff',
       plot = collabs_per_year_plot,
       device = 'tiff',
       path = save_fig_here
       )
...
## Share of total publications per year (mixed collaborations)
```{r}
#' Calculate the cumulative percent of total mixed collaboration studies over the period
papers_per_year_split <- compact_data_with_citation_keys_no_fmd %>%
    filter(year_aggreg != 2005) %>%
    mutate(year_splits = as_factor(ifelse(year_aggreg <= 2013, 'pre_2013', 'post_2013'))) %>% 
    count(year_splits, collab_type, name = 'total_publications') %>%  
    pivot_wider(names_from = year_splits, values_from = total_publications) %>%
    adorn_totals() %>% 
    adorn_percentages(denominator = 'row') %>% 
    adorn_pct_formatting() %>% 
    adorn_ns()

kableExtra::kable(papers_per_year_split, caption = 'Studies per collaboration type pre- and post-2013 (1970-2005) removed')
# share_of_total <- collabs_per_year %>%
#     ggplot() + 
#     geom_bar(aes(x = year_aggreg, y = contribution_cumulative), stat = 'identity') + 
#     geom_hline(aes(yintercept = 0.5)) +
#   scale_x_continuous(breaks = seq(2005, 2020, 2),
#                      labels = x_axis_labels,
#                      expand = c(0, 0)
#                      ) +
#     labs(title = 'Cumulative percentage of total publications per year', 
#          x = 'Year',
#          y = 'cumulative percentage',
#          caption = 'horizontal line shows the 50% mark. The first bar to cross it shows the year that cumulative studies exceeded 50%.')
# 
...
knitr::kable(author_connectedness_to_location_fmd, caption = 'Studies with at least one author affiliation in the location studied', 
             format = "latex", booktabs = TRUE) %>% 
        kableExtra::kable_styling(latex_options = c('striped', 'HOLD_position'))
```
