...
# B. How often are the authors geographically connected to the lcoation studied?
...
```{r}
author_location_count_fmd_wide <- compact_data %>%
  filter(disease == 'fmd') %>% 
  mutate(author_in_country_studied = as_factor(author_in_country_studied), 
         disease = as_factor(disease)
         ) %>% 
...
knitr::kable(author_location_count_fmd_wide)
```
