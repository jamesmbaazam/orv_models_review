...
countries_studied_count <- long_data_no_fmd %>% 
    ...
    ungroup(paper_title) %>% 
    count(country_studied, sort = T) %>% 
    mutate(continent = countrycode(country_studied, 
                                          destination = 'continent',
                                          origin = 'iso2c'
                                          ),
           continent = case_when(is.na(continent) ~ country_studied,
                                 !is.na(continent) ~ continent
                                 )
                                 
           ) %>% 
  mutate(continent = if_else(str_detect(continent, 'asia') == T, 'Asia',
                               if_else(str_detect(continent, 'africa') == T, 'Africa',
                                       continent
                               )
                             )
         )

knitr::kable(countries_studied_count, col.names = c('Country/Setting', 'Number of times studied', 'Continent'))
  
countries_studied_most <- countries_studied_count %>% 
    dplyr::filter(country_studied != 'none') %>% 
    filter(n == max(n))
    
knitr::kable(countries_studied_most, col.names = c('Most studied country', 'Number of times studied', 'COntinent'))


continent_studied_most <- countries_studied_count %>% 
    dplyr::filter(continent != 'none') %>% 
    filter(n == max(n))
    
knitr::kable(continent_studied_most %>% select(-country_studied), col.names = c('Most studied continent', 'Number of times studied'))
```
